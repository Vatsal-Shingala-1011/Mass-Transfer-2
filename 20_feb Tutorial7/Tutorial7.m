...
for i=1:stages
    ...
    [ybe(i+1),yce(i+1)] = vpasolve([y == poly2sym(p1), y == m4 * x + b4],[x,y],[.3 1;0 .4]); %range=[x1 x2;y1 y2]
...
% plot(yce, '-bo');
% xlabel("Number of stages");
% ylabel("Solute Fraction in extract");
% title("Solute Fraction in extract vs no of stages");
% 
% disp("from the graph 0.04 fraction of c remain in raffinat after 10 stages")
