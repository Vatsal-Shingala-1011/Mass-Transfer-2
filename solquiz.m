...
% Calculating slope through interpolation
if ((0 < My) && (My <= 0.04))
...
elseif ((0.083 < My) && (My <= 0.13))
    slope = tie_slope(2) + (My - 0.083)*tie_slope(3)/(0.13 - 0.083);    
    
elseif ((0.13 < My) && (My <= 0.215))
    slope = tie_slope(3) + (My - 0.13)*tie_slope(4)/(0.215 - 0.13);    
    
elseif ((0.215 < My) && (My <= 0.395))
    slope = tie_slope(4) + (My - 0.215)*tie_slope(5)/(0.395 - 0.215);    
...
% Draw the line with the calculated slope through the point (Mx, My)
...
% Highlight the point My, Mx
plot(Mx, My, 'kp', 'MarkerSize', 10, 'MarkerFaceColor', 'g');

F1 = [0, 0.35];
S1 = [0.98, 0.02];
plot(S1,F1,'g^-','linewidth',0.35)
text(0,0.35,'F')
...
for i = 1:stages
    ...
    syms x y
    [xbr(i),xcr(i)] = vpasolve([y == poly2sym(p1),y == My(i) + slope*(x - Mx(i))],[x,y],[0 0.16; 0 0.405]);
    [ybe(i),yce(i)] = vpasolve([y == poly2sym(p2),y == My(i) + slope*(x - Mx(i))],[x,y],[0.16 1; 0 0.405]);
...
disp('raffinate weights (in kg) :')
disp(double(r));
disp('extract weights (in kg) :')
disp(double(e));
disp('raffinate xC,R :')
disp(double(xcr));
disp('extract yC,E :')
disp(double(yce));
disp('total TMA leaving the stages (in kg) :')
disp(double(sum(e.*yce)));
disp('total TMA entering three stages (in kg) :')
disp(double(0.02*sum(S)));
disp('net TMA removed :')
disp(double(sum(e.*yce) - 0.02*sum(S)));
disp('total amount of solute in feed : 350 kg')
disp('fraction of TMA removed  from feed(in percentage) :')
disp(double((sum(e.*yce) - 0.02*sum(S))*100/350));
